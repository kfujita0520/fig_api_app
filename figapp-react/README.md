...
- **Activity tab** – Stake and unstake history from **Solana RPC** (`getSignaturesForAddress` + `getTransaction`). Filters Stake Program instructions (Delegate, Withdraw, Deactivate), shows date, type, amount, status; optionally enriched with current status from Figment stakes. Explorer link per row. No localStorage—same history on any machine for the same wallet.
...
- **Solana RPC** – `getSignaturesForAddress`, `getTransaction` (with `maxSupportedTransactionVersion: 0`) to build activity history; wallet balance via `connection.getBalance`.
